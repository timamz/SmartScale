...
REFERENCE_STORAGE_PATH = os.getenv("REFERENCE_STORAGE_PATH", "/data/reference")
...
app = FastAPI(title="SmartScale API", version="2.0.0")

os.makedirs(REFERENCE_STORAGE_PATH, exist_ok=True)
app.mount("/static/reference", StaticFiles(directory=REFERENCE_STORAGE_PATH), name="reference")
...
class PredictionResult(BaseModel):
    ...
    top_k: list[RetrievalMatch] | None
        ...
        for item in raw_top_k:
            ...
            top_k_models.append(
                RetrievalMatch(
                    ...
                    image_url=f"/static/reference/{image_path}",
                )
            )
...
@app.post("/v1/reference/upload", response_model=ReferenceUploadResponse)
async def upload_reference(
    ...
    label_dir = os.path.join(REFERENCE_STORAGE_PATH, safe_label)
    os.makedirs(label_dir, exist_ok=True)

    ext = os.path.splitext(image.filename or "")[1] or ".jpg"
    file_id = uuid.uuid4().hex[:8]
    relative_path = f"{safe_label}/{file_id}{ext}"
    full_path = os.path.join(REFERENCE_STORAGE_PATH, relative_path)
    ...
    row = db.execute(
        text(
            """
            INSERT INTO reference_images (caption, label, image_path)
            VALUES (:caption, :label, :image_path)
            RETURNING id
            """
        ),
        {"caption": caption, "label": label, "image_path": relative_path},
    ).mappings().first()
    db.commit()

    ref_id = row["id"]
    celery_app.send_task("worker_tasks.embed_reference", args=[ref_id])

    logger.info(
        "reference_uploaded",
        extra={"ref_id": ref_id, "caption": caption, "label": label},
    )

    return ReferenceUploadResponse(ref_id=ref_id, status="processing")
...
def list_references(
    ...
    for r in rows:
        items.append({
            ...
            "caption": r["caption"],
            "label": r["label"],
            "image_url": f"/static/reference/{r['image_path']}",
            "embedded": r["embedded"],
